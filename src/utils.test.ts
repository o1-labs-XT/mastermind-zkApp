...
describe('Provable utilities - unit tests', () => {
  ...
  describe('clue de/serisalization tests', () => {
    it('should correctly pack and unpack 3 clues', () => {
      const clues = [
        [2, 0, 0, 1],
        [1, 2, 0, 0],
        [2, 2, 2, 2],
      ...
      const deserializedClues = serializedClues.map(deserializeClue);

      expect(deserializedClues).toEqual(clues);
    });

    it('should correctly pack and unpack a clue history of 13 elements', () => {
      const clues = Array.from({ length: 13 }, () => [1, 2, 1, 0].map(Field));
      const serializedClues = clues.map(serializeClue);
      const deserializedClues = serializedClues.map(deserializeClue);
...
describe('Tests for dynamic indexing & updating of field arrays', () => {
  describe('getElementAtIndex', () => {
    it('should return the same elements as JS array indexing', () => {
      ...
      for (let i = 0; i < fieldArray.length; i++) {
        expect(getElementAtIndex(fieldArray, Field(i))).toEqual(fieldArray[i]);
      }
    });
  ...
  describe('updateElementAtIndex', () => {
    it('should correctly update an element at the specified index', () => {
      ...
      expect(getElementAtIndex(updatedArray, indexToUpdate)).toEqual(newValue);
